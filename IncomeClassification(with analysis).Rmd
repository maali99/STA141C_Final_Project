...
predict_df_m1<-as.data.frame(predicted)
...
**Customized Prediction Accuracy Calculator**  
Our team wanted to calculate how accurate our models were, but this proved to be difficult because we were trying to implement bootstrapping throughout this project. In the case in which we don’t use bootstrapping and simply use the glm or tree function once without repetition, we could easily use the confusionMatrix function to get the accuracy of our results by testing the respective models on our testing data. With bootstrapping, we would need to create a method that deals with the complexity of our data structure that have gone through transformations. Through a series of simple if else statements we were able to create two functions, one for bootstrapping with the logistic regression model and another for bootstrapping with classification/regression trees. This allowed us to calculate the accuracy of our models. 

The following is an accuracy caculator for Bag of Little Bootstraps for Logistic Regression. It returns a vector of the number of:  
...
After implementing the accuracy calculator we got the following accuracy rates (in %) for the mean, median, and confidence intervals for the Bag of Little Bootstraps with the General Logistic Regression Model. 
```{r}
library(formattable)
...
accuracy_rates<-as.data.frame(t(c(glm_rate_full, mean_rate, median_rate, ci1_rate, ci2_rate, glm_rate_m1))) %>% 
